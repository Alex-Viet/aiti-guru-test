...
function sortByBrand(
  items: Awaited<ReturnType<typeof getProducts>>["products"],
  order: "asc" | "desc",
) {
  ...
  return [...items].sort((a, b) => {
    ...
    const brandCompare = aBrand.localeCompare(bBrand, "en", {
      sensitivity: "base",
    });
...
export function useProducts() {
  ...
  const query = useQuery({
    ...
    queryKey: [
      "products",
      { page: useClientBrandSort ? 1 : page, search, sortBy, order },
    ],
    queryFn: () => {
      if (search.trim()) {
